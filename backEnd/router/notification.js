...
router.post('/accept', async (req, res) => {
  ...
  if (isCollection) {
    collection = await collectionModel.findOne({where: {id: idOrder}}).then((order) => {
  ...
  let tariff = await tariffModel.findOne({where: {name: customer.tariffName}})
    ...
    .catch(err => {
      res.json({msg: 'tariff doesn\'t exist', err})
    });
  ...
  if (isCollection) {
    ...
    orderForSmth = await orderModel.findAll({where: {id: idOrderFetch}})
      .then(orders => {
        orders.forEach(el => {
          let price = (el.urgency) ?
            Math.floor(el.price * coeff * 1.2) :
            Math.floor(el.price * coeff);
          payment += price;
        });

        if (payment > customer.coins) res.json({msg: 'customer doesn\'t have money'});
        else {
